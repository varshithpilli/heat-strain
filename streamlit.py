...
def predict_risk(model, feat_row, feature_names, model_name=None, target=None):
    ...
    if model_name == "gat":
        # Reconstruct graph node features
        node_features = torch.tensor(
            np.stack([X[0], X[0] ** 2, np.abs(X[0])], axis=1),
            dtype=torch.float
        )
        # Create fully identical top-4 edge_index
        num_nodes = X.shape[1]
        ...
        with col_res:
            ...
            for target in ["heat_stress_label", "dehydration_label"]:
                if model_sel in models.get(target, {}):
                    ...
                    p, pr = predict_risk(models[target][model_sel], feat, feats, model_name=model_sel, target=target)
            ...
            if heat_pred is not None:
                ...
                if overall == 2:
                    ...
                    message = (
                        f"⚠️ Health Alert ⚠️\n\n"
                        f"Risk Level: HIGH\n\n"
                        f"Heat Risk: {proba_h[heat_pred] * 100:.2f}%\n\n"
                        f"Dehydration Risk: {proba_d[dehyd_pred] * 100:.2f}%\n\n"
